...
class ViewController: UIViewController {
    ...
    let defaultDistanceCameraToObjects = Float(10)
    ...
    @IBAction func chooseObject(_ button: UIButton) {
        ...
        let rowHeight = 45
        let popoverSize = CGSize(width: 250, height: rowHeight * VirtualObjectSelectionViewController.countObjects)
    ...
    @IBAction func speakTapped(_ sender: UIButton) {
        ...
        guard let settingsViewController = storyboard.instantiateViewController(withIdentifier: "speechController") as? SpeechController else {
            return
        }
        
        let barButtonItem = UIBarButtonItem(barButtonSystemItem: .done, target: self, action: #selector(dismissSpeakVC))
        settingsViewController.navigationItem.rightBarButtonItem = barButtonItem
        settingsViewController.title = "Voice Assistant"
        
        let navigationController = UINavigationController(rootViewController: settingsViewController)
        navigationController.modalPresentationStyle = .popover
        navigationController.popoverPresentationController?.delegate = self
        navigationController.preferredContentSize = CGSize(width: sceneView.bounds.size.width - 80, height: sceneView.bounds.size.height - 50)
        self.present(navigationController, animated: true, completion: nil)
        
        navigationController.popoverPresentationController?.sourceView = speakButton
        navigationController.popoverPresentationController?.sourceRect = speakButton.bounds
    }
    
    @objc func dismissSpeakVC() {
...
extension ViewController {
    ...
    func setNewVirtualObjectPosition(_ pos: SCNVector3) {
        ...
        cameraToPosition.setMaximumLength(defaultDistanceCameraToObjects)
